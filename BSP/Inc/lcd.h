...
void Lcd_Draw_Bline(uint16_t x1, uint16_t y1, uint16_t x2, uint16_t y2, uint8_t size, uint16_t color);  //清空屏幕并在右上角显示"RST"
...
void LCD_Show_Binary_Data(uint16_t x, uint16_t y, uint8_t *data, uint16_t len, uint8_t font_size);  // 二进制
...
void LCD_Show_Chinese16(uint16_t x, uint16_t y, uint16_t gbk);
void LCD_Show_ChineseString16(uint16_t x, uint16_t y, uint8_t *str);

#endif //CPROJECT_LCD_H
